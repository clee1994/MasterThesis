...
# taking past and future to correctly estimate var / cov
def data_label_method_cov(lreturns, cur_d, dates_stocks):
	...
	try:
		ret_val = np.cov(lreturns[0, (indt - n):(indt + n)], 
			lreturns[1, (indt - n):(indt + n)])[0, 1]
	except:
...
def gen_xy_daily(documents, features, window, mcount, dm_opt, tables = False,
 dmm = 0, dmc = 0):
	import datetime
	import numpy as np
	...
	x = []
	model = Doc2Vec(dm = dm_opt, size = features, window = window, 
		min_count = mcount, workers = 4, dbow_words = 1, dm_mean = dmm, 
		dm_concat = dmc)
...
def bench_mark_cov(lreturns, dates_stocks, n_past, len_o):
	...
	for i in len_o:
		indt = ind_closest(i, dates_stocks)
		ret_val.append(np.cov(lreturns[0, (indt - n_past):indt], lreturns[1, 
			(indt - n_past):indt])[0, 1])
...
def calibrate_doc2vec(documents, y, dates_ret, test_split):
	...
	for fts in fts_space:
		x = gen_xy_daily(documents, fts, ws_def, mc_def, dm_def, False, 
			dmm_def, dmc_def)
	...
	loss = []
	for ws in ws_space:
		x = gen_xy_daily(documents, fts_opt, ws, mc_def, dm_def, False, 
			dmm_def, dmc_def)
		loss.append(val_cv_eval(x, y, test_split))
		print('doc2vec model built', flush = True)
	...
	loss = []
	for mc in mc_space:
		x = gen_xy_daily(documents, fts_opt, ws_opt, mc, dm_def, False, 
			dmm_def, dmc_def)
		loss.append(val_cv_eval(x, y, test_split))
		print('doc2vec model built', flush = True)
	...
	for dm in x_dm_space:
		x = gen_xy_daily(documents, fts_opt, ws_opt, mc_opt, dm, False, 
			dmm_def, dmc_def)
		loss.append(val_cv_eval(x, y, test_split))
		print('doc2vec model built', flush = True)
	...
	for dmm in x_dmm_space:
		x = gen_xy_daily(documents, fts_opt, ws_opt, mc_opt, dm_opt, False, 
			dmm, dmc_def)
		loss.append(val_cv_eval(x, y, test_split))
		print('doc2vec model built', flush = True)
	...
	parameters = 'Doc2Vec: features ' + str(fts_opt) + ', window ' + 
		str(ws_opt) + ', min. count ' + str(mc_opt) + 
		(', PV-DM, ' if dm_opt else ', PV-DBoW, ') + 
		('mean, ' if dm_opt else 'sum') + ('' if dm_opt else 'concatenation')
...
def sort_predictability(news_data, lreturns, dates, test_split, names):
	...
	[documents, y, dates_x] = create_documents(news_data, lreturns, dates, 
		data_label_method_val)
	...
	for i in range(np.shape(y)[1]):
		...
		if np.sum(ind_mask) > 0:
			scores = cross_val_score(clf, x_temp[ind_mask, :], y[ind_mask, i], 
...
def produce_y_ret(returns, dates_prices, dates_news):
	...
	dates_news = list(map(datetime.datetime.date, 
		dates_news.astype(datetime.date)))
...
def produce_y_cov(returns, dates_prices, dates_news):
	...
	for i in dates_news:
		...
		try:
			ret_val = np.cov(returns[0, (indt - n):(indt + n)], 
				returns[1, (indt - n):(indt + n)])[0, 1]
		except:
...
def append_past_obs_cov(x, dates_news, lreturns, dates_prices):
	...
	for i in range(len(dates_news)):
		...
		for j in np.arange(n_reach, 0, -1):
			x_ret[i, -j] = data_label_method_cov(np.transpose(lreturns), 
				dates_news[i], dates_prices)

	return x_ret


def produce_mu_cov(x, test_split, lreturns, dates_prices, dates_news, n_past,
 names, firm_ind_u, reg_method):
	import numpy as np
	import datetime
	...
	for i in firm_ind_u:
		...
		if i = = firm_ind_u[0]:
			stables = True
			[mu_p_ts_temp, lossest, r2t, parameters_reg] = reg_method(x, y, 
				test_split, temp1, dates_prices, dates_news, n_past, i, 
				bench_mark_mu, "Mean", names[i], show_p, stables)
		else:
			[mu_p_ts_temp, lossest, r2t, _] = reg_method(x, y, test_split, 
				temp1, dates_prices, dates_news, n_past, i, bench_mark_mu, 
				"Mean", names[i], show_p, stables)
		mu_p_ts = np.concatenate([mu_p_ts, np.reshape(mu_p_ts_temp, 
			(np.shape(mu_p_ts)[0], 1))], axis = 1)
		r2_mat.append(r2t)
		losses = losses + lossest
		print(str(datetime.datetime.now()) + 
			': Successfully produced mu_p_ts for ' + names[i], flush = True)
		if i = = firm_ind_u[0]:
			stables = False


	# get improved cov estimates
	cov_p_ts = np.zeros([int(np.ceil(np.shape(x)[0] * test_split)), 
		len(firm_ind_u), len(firm_ind_u)])
	for i in range(len(firm_ind_u)):
		for j in range(i + 1):
			...
			if past_obs_int:
				x = append_past_obs_cov(x, dates_news, lreturns[:, [i, j]], 
					dates_prices)
			[cov_p_ts[:, i, j], lossest, r2t, _] = reg_method(x, y, test_split, 
				temp1, dates_prices, dates_news, n_past, i, bench_mark_cov, 
				label_text, l2_test , show_p, stables)
			cov_p_ts[:, j, i] = cov_p_ts[:, i, j]
			losses = losses + lossest
			r2_mat.append(r2t)
			print(str(datetime.datetime.now()) + 
				': Successfully produced co_p_ts for ' + names[firm_ind_u[i]] + 
				' and ' + names[firm_ind_u[j]], flush = True)
			if (i = = j) and (i = = 0):
				stables = False
			if (i = = 0) and (j = = 1):
				stables = False
...
#TFIDF and n-grams
def create_corpus(n_gram, news_data, lreturns, dates_stocks, test_split,
 tfidf_bow):
	import numpy as np
...
def tfidf_vector(n_gram, corpus, lreturns, dates_stocks, test_split,
 tfidf_bow, x_dates):
	import numpy as np
	from sklearn.feature_extraction.text import TfidfVectorizer
	from sklearn.feature_extraction.text import CountVectorizer
	...
	for i in fts_space:
		if tfidf_bow:
			vectorizerCount = CountVectorizer(analyzer = 'word', 
				ngram_range = (1, n_gram), stop_words = 'english', min_df = 3, 
				max_features = i)
			x = vectorizerCount.fit_transform(corpus).toarray()
			y = np.array([data_label_method_val(lreturns, cur_d, 
				dates_stocks) for cur_d in x_dates ])
			lossC.append(val_cv_eval(x, y, test_split))
			parameter1.append('BoW: stopwords, cut-off = 3, features = ' + 
				str(i) + ', ' + str(n_gram) + '-gram')

		else:
			vectorizerTFIDF = TfidfVectorizer(analyzer = 'word', 
				ngram_range = (1, n_gram), stop_words = 'english', min_df = 3, 
				max_features = i)
			...
			y = np.array([data_label_method_val(lreturns, cur_d, 
				dates_stocks) for cur_d in x_dates ])
			lossC.append(val_cv_eval(x, y, test_split))
			parameter1.append('TFIDF: stopwords, cut-off = 3, features = ' + 
				str(i) + ', ' + str(n_gram) + '-gram')
		del x, y
		gc.collect()
	...
	if tfidf_bow:
		vectorizerCount = CountVectorizer(analyzer = 'word', 
			ngram_range = (1, n_gram), stop_words = 'english', min_df = 3, 
			max_features = fts_space[np.argmax(lossC)])
		x = vectorizerCount.fit_transform(corpus).toarray()
	else:
		vectorizerCount = TfidfVectorizer(analyzer = 'word', 
			ngram_range = (1, n_gram), stop_words = 'english', min_df = 3, 
			max_features = fts_space[np.argmax(lossC)])
		x = vectorizerCount.fit_transform(corpus).toarray()
...
def val_cv_eval(x, y, split):
	...
	clf = LinearRegression(n_jobs = number_jobs)
	if np.sum(ind_mask) > 0:
		scores = cross_val_score(clf, x[ind_mask, :], y[ind_mask], cv = 5, 
			scoring = 'neg_mean_squared_error', n_jobs = number_jobs)
		return scores.mean()
	else:
		return 0

def estimate_linear(x_cal, y_cal, test_split, lreturns, dates, x_dates,
 n_past, ind_r, benchm_f, mu_var, t_text, show_plots, tables):
	from sklearn.linear_model import LinearRegression
	...
	x_train, y_train, x_test, y_test = train_test_split(x_cal, y_cal, 
		test_split)

	#benchmark estimates
	bench_y = benchm_f(lreturns, dates, n_past, 
		x_dates[np.shape(x_dates)[0] - np.shape(y_test)[0]:np.shape(x_dates)[0]])
	...
	#set variance to zero if negativ
	...
	if tables:
		...
		learning_curve_plots(grid_results, clf, x_cal, y_cal, n_cpu, 
			alpha_range, gamma_range, show_p)

	parameters = 'linear Regression'
	return mu_p_ts, losses, r2, parameters

def estimate_ridge(x_cal, y_cal, test_split, lreturns, dates, x_dates,
 n_past, ind_r, benchm_f, mu_var, t_text, show_plots, tables):
	...
	#alpha_range = np.geomspace(0.1, 80, 12)
	...
	gamma_range = [ 1.00000000e-02,   2.19852420e-02,   4.83350864e-02, 
					...
					1.20000000e+01]
	RR_parameters = [{'kernel': ['rbf'], 'gamma': gamma_range, 
		'alpha': alpha_range1}]

	RR_model = KernelRidge(alpha = 30)
	clf = GridSearchCV(RR_model, RR_parameters, 
		scoring = 'neg_mean_squared_error', n_jobs = number_jobs)
	...
	if tables:
		plot_pred_true_b(grid_results, clf, x_cal, y_cal, 4, alpha_range1, 
			gamma_range, y_test, mu_p_ts, bench_y, mu_var, t_text)
		learning_curve_plots(grid_results, clf, x_cal, y_cal, 4, alpha_range1, 
			gamma_range, show_plots)


	para_string = 'Ridge Regression:'
	for key, value in clf.best_params_.items():
		if type(value) = = np.float64:
			para_string = para_string + ' ' + key + '=' + 
				"{:.4f}".format(value) + ', '
...
def estimate_SVR(x_cal, y_cal, test_split, lreturns, dates, x_dates, n_past,
 ind_r, benchm_f, mu_var, t_text, show_plots, tables):
...
def estimate_xgboost(x_cal, y_cal, test_split, lreturns, dates, x_dates,
 n_past, ind_r, benchm_f, mu_var, t_text, show_plots, tables):
	...
	clf = GridSearchCV(xgb_model, {'max_depth': max_depth_range, 
		'n_estimators': n_est_range}, n_jobs = number_jobs)
	clf = clf.fit(x_train[ind_mask[:, 0], :], y_train[ind_mask[:, 0]])
...
def estimate_keras(x_cal, y_cal, test_split, lreturns, dates, x_dates,
 n_past, ind_r, benchm_f, mu_var, t_text, show_plots, tables):
	...
	x_train, y_train, x_test, y_test = train_test_split(
		x_cal[ind_mask[:, 0], :], y_cal[ind_mask[:, 0]], test_split)
	...
	model.fit(x_train[:, :, None], y_train, 
		validation_data = (x_test[:, :, None], y_test), epochs = 5, 
		batch_size = 64)
